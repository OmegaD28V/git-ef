<?php
    ...
    if (!isset($_GET["provD"])) {
        ...
        $domicilio = MvcController::clienteDomicilioController($prov, "prov");
        $phone = MvcController::clientePhoneController($prov, "prov");
...
<div class="contenedor-formulario">
    <form class="formulario-editar" method="post" name="editarCliente">
        ...
        <div class="input-group">
            ...
            <input type="text" name="name-user" id="name-user" value="<?=$proveedor["nombre"]?>" required>
        </div>
        <div class="input-group">
            <label for="ape-user">Usuario</label>
            <div class="input-disabled"><span id="correo-user"><?=$proveedor["usuario"]?></span></div>
        </div>
        <div class="input-group">
...
<?php
        $gestor = 33;
        $actualizar = MvcController::actualizarUsuarioController($prov, $idDomicilio, $idPhone, $gestor);
        if (isset($_GET["err"])) {
            if ($_GET["err"] == "ur") {
                ?>
