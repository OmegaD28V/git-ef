...
<div class="contenedor-formulario">
    ...
    <?php if($noProductos["total"] <= $articulos){}else{ ?>
        <div class="pnt">
            <a class="pnt__previous <?=$_GET["pag"] <= 1 ? 'disabled' : '' ?>" id="prev" href="index.php?action=productoInicio&pag=<?=$_GET["pag"]-1?>">< Anterior</a>

            <?php for($i = 1; $i <= $paginas; $i++):?>
                <a class="pnt__pag <?=$_GET["pag"] == $i ? 'active' : '' ?>" href="index.php?action=productoInicio&pag=<?=$i?>"><?=$i?></a>
            <?php endfor ?>
            
            <a class="pnt__next <?=$_GET["pag"] >= $paginas ? 'disabled' : '' ?>" id="next" href="index.php?action=productoInicio&pag=<?=$_GET["pag"]+1?>">Siguiente ></a>
        </div>
    <?php } ?>
